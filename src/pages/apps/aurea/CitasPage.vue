...
async function load() {
  ...
  try {
    const { data } = await api.get('/apps/aurea/citas')
...
async function loadServicios() {
  try {
    const { data } = await api.get('/apps/aurea/servicios')
  ...
  try {
    ...
    if (form.value.id) {
      ...
      for (const payload of citasParaCrear) {
        await api.post('/apps/aurea/citas', payload)
      }
...
async function confirmarPago() {
  ...
  try {
    ...
    await api.post('/apps/aurea/pagos', payload)
